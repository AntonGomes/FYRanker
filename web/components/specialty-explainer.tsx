        ...
        {PHASES.map((phase, i) => (
          ...
          >
            ...
            <div className="pt-1.5 space-y-1">
              <h3 className="text-sm font-semibold text-foreground leading-tight">{phase.title}</h3>
...
function InfoCard({ specialtyCount }: { specialtyCount: number }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="flex gap-2.5">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          <span className="font-medium text-foreground">{specialtyCount} specialties</span>{" "}
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center px-2 py-4">
      <div className="w-full max-w-sm space-y-6">
        <PhaseTimeline />
        <InfoCard specialtyCount={specialtyCount} />
      </div>
    </div>
  );
}
