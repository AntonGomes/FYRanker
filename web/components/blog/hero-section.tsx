...
function HeroHeadline({ isInView }: { isInView: boolean }) {
  return (
    <motion.h1
...
function HeroCTA({ isInView }: { isInView: boolean }) {
  return (
    <motion.div
      className="mt-8"
...
export function HeroSection({ data }: HeroSectionProps) {
  ...
  return (
    ...
    >
      <HeroHeadline isInView={isInView} />
      <div className="mt-12 sm:mt-16 flex flex-wrap items-center justify-center gap-8 sm:gap-12 md:gap-16">
        <AnimatedStat value={data.rotations} label="rotations" trigger={isInView} delay={0.3} />
        <AnimatedStat value={data.sites} label="sites" trigger={isInView} delay={0.5} />
        <AnimatedStat value={data.specialties} label="specialties" trigger={isInView} delay={0.7} />
      </div>
      <motion.p
